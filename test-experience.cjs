...
(async()=>{
 ...
 assert.match(await page.locator('main').innerText(),/AI learns patterns/);
 ...
 await page.getByRole('button',{name:'Just be helpful',exact:true}).click();
 assert.match(await page.locator('#detail-feedback').innerText(),/vague/);
 await page.getByRole('button',{name:'Explain in two short sentences with one example',exact:true}).click();
