...
window.EveRealtime=(()=>{
 ...
 function microphone(){stream?.getAudioTracks().forEach(t=>t.enabled=!busy());}
 ...
 async function onEvent(event,token){
  ...
  if(event.type==='conversation.item.input_audio_transcription.completed'&&event.transcript?.trim()&&!audioSpeaking){
 ...
 async function connect(payload){
  ...
  const token=epoch;connecting=(async()=>{
   ...
   if(token!==epoch)return;voiceSession.phase='listening';voiceSession.enabled=true;lastActivity=Date.now();const started=Date.now();watch=setInterval(()=>{if(!busy()&&(Date.now()-lastActivity>90000||Date.now()-started>900000)){stop();paint();}},5000);paint();
 ...
 async function guide(d,message,transition=null,exact=false){
  if(!enabled()||!voiceSession.enabled||!CourseSession.active)return;claimEveTeachingEvent(d);if(busy()){pendingGuide=[d,message,transition,exact];return;}day=d;const rev=++revision,payload=evePayload(d,message,'guidance',transition);awaiting=true;microphone();
  try{await connect(payload);if(rev!==revision||!pc)return;context=await api('/v1/teacher/realtime/context',payload);if(rev!==revision||!pc)return;send({type:'session.update',session:{type:'realtime',instructions:context.instructions,audio:{input:{turn_detection:context.turn_detection}}}});
   ...
   send({type:'response.create',response:{instructions:context.instructions+'\nFor this turn only: '+(exact?'Speak this checked feedback exactly, without another question: ':'Teach this current event in your own words, not by reading instructions: ')+message,output_modalities:['audio']}});microphone();paint();
