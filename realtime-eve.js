...
window.EveRealtime=(()=>{
 let pc=null,dc=null,stream=null,audio=null,connecting=null,epoch=0,revision=0,day=0,spokenDay=0,context=null,responseActive=false,paused=false,awaiting=false,events=[],lastActivity=0,watch,callId=null,closing=Promise.resolve();
 ...
 function paint(){if(!enabled()||typeof voiceSession==='undefined')return;const orb=document.getElementById('eve-orb');if(orb){orb.disabled=false;orb.textContent=pc?'Stop Eve':voiceSession.phase==='error'?'Try again':'Talk to Eve';orb.setAttribute('aria-label',orb.textContent);orb.onclick=()=>pc?stop():startForPage();}let controls=document.getElementById('eve-live-controls');if(!controls&&orb){controls=document.createElement('div');controls.id='eve-live-controls';controls.className='eve-live-controls';orb.after(controls);}if(controls){controls.replaceChildren();if(pc){const status=document.createElement('span');status.setAttribute('role','status');status.textContent=paused?'Microphone paused':responseActive?'Eve is speaking · You can interrupt':'Listening · Speak naturally';const mute=document.createElement('button');mute.className='button secondary';mute.textContent=paused?'Resume microphone':'Pause microphone';mute.onclick=()=>{paused=!paused;stream?.getAudioTracks().forEach(t=>t.enabled=!paused);paint();};controls.append(status,mute);}else{const status=document.createElement('span');status.textContent=voiceSession.phase==='error'?'Eve could not connect. Try again or continue reading.':'';controls.append(status);}}}
 function interrupt(){if(responseActive)send({type:'response.cancel'});send({type:'output_audio_buffer.clear'});responseActive=false;awaiting=false;events=[];paint();}
 function stop(notify=true){const id=callId;callId=null;epoch++;revision++;clearInterval(watch);watch=null;events=[];awaiting=false;responseActive=false;connecting=null;pc?.close();dc?.close();stream?.getTracks().forEach(t=>t.stop());if(audio){audio.pause();audio.srcObject=null;audio.remove();}pc=dc=stream=audio=null;paused=false;if(typeof voiceSession!=='undefined'){voiceSession.generation++;voiceSession.enabled=false;voiceSession.phase='idle';}paint();if(id&&notify&&enabled()&&CourseSession.active)closing=api('/v1/teacher/realtime/stop',{call_id:id}).catch(()=>{});}
 async function onEvent(event,token){
  if(token!==epoch)return;lastActivity=Date.now();
  if(event.type==='input_audio_buffer.speech_started'){responseActive=false;events=[];awaiting=false;paint();}
  if(event.type==='response.created'){responseActive=true;spokenDay=day;awaiting=false;paint();}
  if(event.type==='output_audio_buffer.started'){responseActive=true;project().guidance.eveGreeted=true;save();paint();}
  if(event.type==='output_audio_buffer.stopped'||event.type==='output_audio_buffer.cleared'){responseActive=false;paint();}
