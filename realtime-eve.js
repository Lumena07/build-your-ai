...
window.EveRealtime=(()=>{
 let pc=null,dc=null,stream=null,audio=null,connecting=null,epoch=0,revision=0,day=0,spokenDay=0,context=null,responseActive=false,awaiting=false,events=[],lastActivity=0,watch,callId=null,closing=Promise.resolve();
 ...
 function paint(){if(!enabled()||typeof voiceSession==='undefined')return;const orb=document.getElementById('eve-orb');if(orb){orb.disabled=false;orb.textContent='Talk to Eve';orb.setAttribute('aria-label','Talk to Eve');orb.setAttribute('aria-pressed',String(!!pc));orb.setAttribute('aria-describedby','eve-live-controls');orb.title=pc?'Click again to stop Eve':'Start talking with Eve';orb.onclick=()=>pc?stop():startForPage();}let controls=document.getElementById('eve-live-controls');if(!controls&&orb){controls=document.createElement('div');controls.id='eve-live-controls';controls.className='eve-live-controls';orb.after(controls);}if(controls){controls.replaceChildren();const status=document.createElement('span');status.setAttribute('role','status');status.textContent=pc?(responseActive?'Eve is speaking. ':'Listening. ')+'Speak anytime to interrupt Eve. Click again to stop.':voiceSession.phase==='error'?'Eve could not connect. Click Talk to Eve to try again, or continue reading.':'Speak anytime to interrupt Eve.';controls.append(status);}}
