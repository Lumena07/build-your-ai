...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>document.querySelector('h1')?.textContent.includes('Inside AI’s Words'));
  ...
  assert.match(request.lesson_connection,/Mission 1 showed a text answer/);
  assert.match(request.opening_action,/Connect Mission 1/);
  assert.match(request.learner_message,/moved from (Day|Mission) 1 to (Day|Mission) 2/);
  ...
  assert.ok(['pattern','example','request','answer','mission 1'].some(word=>spoken.includes(word)),'Eve did not recap Mission 1');
  ...
  console.log(`Mission 1 recap and connection to Mission 2 tokens passed (${live?'live Eve':'mocked Eve'}).`);
