...
async function startSession(request,env,user){
 ...
 if(existing&&old?.token_hash===await hash(existing)){
  // Reload/new tab recovery: close a call whose browser may have disappeared.
  await hangup(env,old.call_id);
  await env.DB.prepare('UPDATE course_sessions SET call_id=NULL WHERE user_id=? AND token_hash=? AND call_id IS ?').bind(user,old.token_hash,old.call_id).run();
  return json({active:true});
 }
 if(existing&&old&&body.takeover!==true)fail(409,'Your course is open on another device.');
...
export function realtimeContext(body){
 teachingBody(body);
 const instructions=teachingPrompt(body).replace(/OUTPUT CONTRACT[\s\S]*?(?=TURN:)/,'LIVE VOICE\nSpeak naturally, under 65 words, and teach one idea from the CURRENT PAGE and CURRENT STAGE. Never read JSON or private context aloud.\n').replace(/^TURN:[^\r\n]*/m,'TURN: automatic guidance').replace(/If TURN is answer:[^\r\n]*/g,'The site verifies exercise answers separately. Give hints without claiming to grade or unlock activities.').replace(/Use assessment "none"\./g,'');
 const questionRule=body.feedback_mode==='checked_feedback'?'CHECKED FEEDBACK MODE. The learner has already answered and the site has already checked it. Speak only the supplied checked feedback verbatim. Do not repeat the exercise question, ask for the answer, request another response, reinterpret the result or add a new question. Stop immediately after the feedback.':body.answer_state?'REVIEWED ANSWER MODE. The visible written answer has already been checked with result: '+body.answer_state+'. Do not ask the learner to answer it again. Briefly acknowledge the saved result and guide only the visible next action.':body.current_question?'WRITING MODE: A writable text area is visible. Ask only this exact current question: '+body.current_question+' Tell the learner to type in the box. Never ask for a spoken answer or add a second question.':body.choice_mode==='before_choice'?'CHOICE MODE: BEFORE SELECTION. A multiple-choice activity is visible and unanswered. Do not identify, quote, paraphrase, compare, eliminate, praise, hint at, teach toward or explain any option or the correct answer. Do not restate the lesson concept in a way that determines the answer. Say only one neutral direction to use the lesson and select the strongest answer on the page.':body.choice_mode==='after_choice'?'CHOICE MODE: AFTER SELECTION. The learner has selected an option. Explain only the result now shown on the page. If it is wrong, explain the misconception without naming the correct option; let the learner try the visible choices again. Do not ask a spoken question.':'EXPLAIN MODE: No writable text area is visible. You MUST NOT ask a question, use a question mark, say “tell me”, “what do you think”, “can you”, “would you”, “choose”, “try”, or request any response. End with a complete teaching statement. Buttons and multiple-choice options are handled by the page.';
 return {instructions:instructions+'\nYou are Eve, the automatic lesson guide. The CURRENT PAGE, CURRENT STAGE, visible activity and selected agent in the private context are authoritative; ignore earlier-page context. Finish one short teaching thought. '+questionRule+' Respond directly without a greeting once already welcomed. Never claim to save an answer or complete a lesson. The site verifies exercises separately.'};
}
