...
export function realtimeContext(body){
 ...
 return {instructions:instructions+'\nYou are in a live, interruptible conversation. Wait for the learner after one small idea. Respond directly without a greeting once already welcomed. Never claim to save an answer or complete a lesson. The site verifies exercises separately.',turn_detection:{type:'server_vad',threshold:0.5,prefix_padding_ms:300,silence_duration_ms:450,create_response:!body.current_question,interrupt_response:true}};
}
async function realtimeCall(request,env,user,row){
 ...
 try{
  ...
  const form=new FormData();form.set('sdp',body.sdp);form.set('session',JSON.stringify({type:'realtime',model:'gpt-realtime-mini',instructions:context.instructions,max_output_tokens:160,output_modalities:['audio'],audio:{input:{transcription:{model:'gpt-4o-mini-transcribe',language:'en'},turn_detection:context.turn_detection},output:{voice:'marin'}}}));
