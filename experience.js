...
function guidedDay1(){
 ...
 const content=[
  `<h2>You may already use AI</h2><p>A phone can recognise your spoken words, a photo app can find faces, and a writing assistant can suggest a reply. These are different AI tasks.</p><p><b>Artificial intelligence (AI)</b> is the broad name for computer systems that perform tasks such as recognising patterns, making predictions and generating content. Many modern AI systems learn from examples.</p><details><summary>Does every computer program use AI?</summary><p>No. An ordinary alarm follows a time you set. A speech recogniser learns patterns that help it handle voices it has not heard before.</p></details><p><b>Predict first:</b> what could a text assistant do with this question?</p><blockquote>${esc(x.question)}</blockquote><button class="button" onclick="advanceDay1(2)">See a sample answer</button>`,
  `<h2>From a question to an answer</h2><p><b>You ask:</b> ${esc(x.question)}</p><p><b>Prepared sample answer:</b> ${esc(x.answer)}</p><p>This is an illustration, not a live generated answer. In a text AI, training builds patterns from many examples. Your current question supplies context; the model uses those patterns to generate a response.</p><details><summary>Where do the answers come from?</summary><p>They are generated using what the model learned during training and the information provided now. It does not automatically search the internet. An answer may reflect learned facts, combine ideas, or contain a mistake.</p></details><button class="button" onclick="advanceDay1(3)">What should I check?</button>`,
  `<h2>Useful does not mean all-knowing</h2><p>Now ask: <b>${esc(x.hard)}</b></p><p>Compare two possible replies:</p><p><b>A:</b> “I know the exact answer. You can rely on me.”</p><p><b>B:</b> “I need more information to answer reliably. Let’s identify what is missing.”</p><p>Both sound fluent. Only one acknowledges the missing evidence. Fluency is a skill; it is not proof.</p><details><summary>Think a little deeper</summary><p>What information would make this question answerable? Would a document help, or is it asking for a future event nobody can guarantee? Explain your reasoning to Eve.</p></details><button class="button" onclick="advanceDay1(4)">Try a small task</button>`,
  `<h2>You decide how to use AI</h2><p>You want help with: ${esc(x.question)}</p><p>Which approach makes better use of what you learned?</p><button class="button secondary" onclick="day1ChooseDetail(false)">Use a fluent answer as evidence that it is correct</button> <button class="button secondary" onclick="day1ChooseDetail(true)">Use the answer as a starting point and check important claims</button><p id="detail-feedback" aria-live="polite"></p><details><summary>Challenge: explain it without jargon</summary><p>Tell Eve what AI is using an example from your chosen assistant. Include one useful task and one limitation. Eve can help you improve the explanation.</p></details><p><b>Your takeaway:</b> AI can recognise patterns and generate useful content. A text assistant uses learned patterns and your question; it does not know everything.</p><button class="button" onclick="finishLab('lab1','lab2')">Check what I learned</button>`
 ];
 return `${header('DAY 1 · WHAT IS AI?','Meet AI: useful, capable, imperfect.','Discover what AI does, where a text answer comes from, and how to judge when it needs checking.')}<div class="card"><p class="eyebrow">PART ${step} OF 4</p>${content[Math.min(3,step-1)]}</div>`;
}
function day1ChooseDetail(correct){
 ...
 eveTeachMoment(1,correct?'The learner chose to use AI as a starting point and check important claims. Ask for one concrete claim they would verify in their preset example.':'The learner treated fluency as evidence of accuracy. Give one plausible mistake in their preset area and invite another attempt.');
}
...
function installExperience(){
 ...
 advanceDay1=step=>{project().guidance.day1Step=step;save();render();eveTeachMoment(1,'The learner moved to the next part of the introduction. Teach the visible idea. Connect it to the previous part, invite a prediction or reason when useful, and avoid repeating definitions they already grasp.');};
 ...
 teacherPageContext=function(day){
  ...
  return `Course connection: ${lessonJourney[page]?.[0]||''}. Treat the learner as a capable adult who is new to AI. Introduce technical words only with an explanation. Ask for predictions, comparisons or reasons instead of repetitive praise. Increase depth if their answer shows understanding; simplify only the confusing part.\nVisible activity: ${visible}\n${baseContext(day)}`;
 };
 stopEve=cancelVoice;
