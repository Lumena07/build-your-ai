...
(async()=>{
 ...
 try{
  const context=await browser.newContext();await context.addCookies([{name:'testStudent',value:'alice',url:base}]);const page=await context.newPage();await page.goto(base);await page.locator('#learner-name').waitFor();
  ...
  await page.reload();await page.getByRole('button',{name:'Resume with Eve',exact:true}).waitFor();assert.match(await page.locator('main').innerText(),/Welcome back, Alice/);
  await context.addCookies([{name:'testStudent',value:'bob',url:base}]);await page.reload();await page.locator('#learner-name').waitFor();assert.equal(await page.evaluate(()=>KEY),'ai102-v1:bob');assert.equal(await page.locator('#learner-name').inputValue(),'');
  ...
  await context.addCookies([{name:'testStudent',value:'alice',url:base}]);await page.reload();await page.getByRole('button',{name:'Continue on this device',exact:true}).click();await page.getByRole('button',{name:'Resume with Eve',exact:true}).waitFor();assert.match(await page.locator('main').innerText(),/Welcome back, Alice/);assert.equal(await page.evaluate(()=>store.page),'lab1');
  ...
  page.once('dialog',dialog=>dialog.accept());await page.getByRole('button',{name:'Restart this learner',exact:true}).click();assert.equal(await page.locator('#learner-name').inputValue(),'');await page.reload();await page.locator('#learner-name').waitFor();assert.equal(await page.evaluate(()=>store.page),'intro');
  await context.addCookies([{name:'testStudent',value:'bob',url:base}]);await page.reload();await page.getByRole('button',{name:'Continue on this device',exact:true}).click();await page.getByRole('button',{name:'Resume with Eve',exact:true}).waitFor();assert.match(await page.locator('main').innerText(),/Welcome back, Bob/);
